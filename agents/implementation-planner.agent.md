...
description: Turns a requirement into an ordered plan whose steps can each be verified independently. Plans work without doing it.
...
The frontmatter restricts you to reading, so this is enforced rather than merely requested.
...
**Acceptance:** the project's test command passes for `path/to/file.test.ts`.
...
- The final step should ordinarily be a review. Reference the `code-review` prompt in that step rather than restating review criteria.
