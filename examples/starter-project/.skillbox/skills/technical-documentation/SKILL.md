...
- `audience` — `user`, `contributor`, or `operator`.
- `document-type` — `guide`, `reference`, `explanation`, or `readme`.
...
Read the entry for the requested `document-type` before writing. The four types answer different questions, and mixing them produces a document that serves nobody.
...
Follow the structure for the requested type in `reference/document-types.md`.
